...
export default function QuizPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!quiz) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 text-center">
        <nav className="text-sm text-charcoal-light mb-8 text-right">
          <Link href="/" className="hover:text-bronze transition-colors">
            الرئيسية
          </Link>
          <span className="mx-2">/</span>
          <Link href="/self-test" className="hover:text-bronze transition-colors">
            اختبر نفسك
          </Link>
        </nav>
        <div className="bg-white rounded-2xl p-8 sm:p-12 border border-cream-dark/30">
          <div className="w-16 h-16 mx-auto rounded-full bg-cream-warm flex items-center justify-center mb-4">
          ...
          <h1 className="text-xl font-bold text-charcoal mb-2">الاختبار غير موجود</h1>
          <p className="text-sm text-charcoal-light mb-6">عذراً، لم يتم العثور على الاختبار المطلوب.</p>
          <Link
            href="/self-test"
            className="inline-flex items-center px-6 py-3 text-sm font-medium text-white bg-bronze hover:bg-bronze-light rounded-xl transition-colors"
          >
            العودة للاختبارات
          </Link>
        </div>
  ...
  const barColor = progressBarColors[quiz.color] || 'bg-bronze';
  ...
  const submitQuiz = async (skip: boolean) => {
    ...
    const resultObj = quiz.results.find(
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <nav className="text-sm text-charcoal-light mb-8">
        ...
        <span className="mx-2">/</span>
        <span className="text-charcoal">{quiz.title}</span>
      </nav>

      <div className="mb-8">
        <h1 className="text-3xl font-bold text-charcoal">{quiz.title}</h1>
